...
Large language models (LLMs) have limited [context windows](https://en.wikipedia.org/wiki/Context_window). The context window is what the model can "see" while producing a response. The [transformer architecture](https://en.wikipedia.org/wiki/Transformer_\(deep_learning\)) used by LLMs limits how much input they can process. The input for a coding agent session includes all the previous messages and tool calls, and this keeps growing as you work. Once it exceeds the context window, the LLM rejects the request.
